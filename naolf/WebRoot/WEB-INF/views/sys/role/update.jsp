...
<body>
                    ...
                    <div class="ibox-content">
                        <form action="" method="post" class="form-horizontal">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="descript"value="${role.description }">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                	<input type="checkbox" class="js-switch" name="available" <c:if test="${role.available==0}">checked</c:if> />
                                </div>
                            </div>
		                            ...
		                            <div class="form-group">
		                                ...
		                                <div class="col-sm-10">
		                                	<c:forEach items="${perm.childPers}" var="childPer">
		                                		<label class="checkbox-inline i-checks"><input type="checkbox" value="${childPer.id}"  name="permbox"  <c:if test="${fn:indexOf(role.perms,childPer.id)>-1}">checked</c:if>>${childPer.name}</label>
		                                	</c:forEach>
		                                </div>
		                            </div>
	                             </c:if>
    ...
    <script>
        ...
        function update(){
        	...
        	$.post("${ctx}/manage/role/update",{"id":'${role.id}',"name":$("[name='name']").val(),"description":$("[name='descript']").val(),"available":available,"perms":perms.toString()},function(data){
					if(data.code=="0"){
						opt_success("修改成功",0);
					}else{
						opt_error("修改失败");
