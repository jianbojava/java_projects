                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:role:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增角色','${ctx}/manage/role/add','','',true)"><i class="fa fa-plus"></i> 新增角色</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="manage:role:del">
                                <div class="btn-group">
							...
							<form action="${ctx }/manage/role/list" method="post" id="searchForm" class="pull-right mail-search" target='_self'>
		                        <div class="input-group">
		                            <input type="text" class="form-control input-sm" name="keywords" value="${vo.keywords}" placeholder="搜索名称、描述等">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                <thead>
                                <tr>
                                	<th width="30px"><input type="checkbox" class="i-checks" name="input"></th>
                                ...
                                <tr>
                                	<td><input type="checkbox" class="i-checks" name="input[]" value="${role.id }"></td>
                                	<td>${role.name }</td>
                                    <td>${role.description }</td>
                                    <td><c:choose><c:when test="${role.available==0}"><span class="label label-primary">启用</span></c:when><c:otherwise><span class="label label-danger">禁用</span></c:otherwise></c:choose></td>
                                    <td>
                                    	<shiro:hasPermission name="manage:role:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改角色','${ctx}/manage/role/update/${role.id}','','',true)"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    	<shiro:hasPermission name="manage:role:del">
                                    	<div class="btn-group">
			                                <a data-toggle="dropdown" class="btn btn-xs btn-white dropdown-toggle"><i class="fa fa-cog"></i> 操作 <span class="caret"></span></a>
			                                <ul class="dropdown-menu">
			                                    <li><a onclick="del(0,'${role.id}')" <c:if test="${role.available==0}">class="font-bold"</c:if>>正常</a></li>
			                                    <li><a onclick="del(1,'${role.id}')" <c:if test="${role.available==1}">class="font-bold"</c:if>>禁用</a></li>
			                                </ul>
			                            </div>
			                            </shiro:hasPermission>
                                    </td>
    ...
    <script>
        ...
        function del(flg,id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/role/del/"+flg+"/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else{
        ...
        function delM(flg){
        	if($("[name='input[]']").filter(':checked').length>0){
        		layer.confirm('确定要操作吗？', function(index){
					  ...
					  $.post("${ctx}/manage/role/del/"+flg+"/"+dels.substring(1),function(data){
						if(data.code=="0"){
							opt_success("操作成功");
						}else{
							opt_error("系统繁忙，请稍后再试");
