...
<div class="container mBtm91">
	<div class="confirm_order">
        ...
        <div class="orderDetail">
        	<input type="hidden" value="${lesson.id }" id="id"/>
	        	<div class="orderName bold f28">${lesson.catname }&nbsp;|&nbsp;${lesson.name }</div>
...
<!-- 	                	<div class="course f24 color_a5a4a8">训练课时:${lesson.period }</div> -->
                        <c:if test="${not empty lesson.period and 0 == lesson.train}"><div class="course f24 color_a5a4a8">训练课时：${lesson.period }天集中训练</div></c:if>
                        <c:if test="${not empty lesson.period and 0 != lesson.train}"><div class="course f24 color_a5a4a8">训练课时：${lesson.period }天集中训练+${lesson.train }课时进阶训练</div></c:if>
            ...
            <div class="orderInfo pTop30">
                ...
                <div class="rowTotal">
    				...
    				<span class="price f30 bold color_745398" value="${ticket.price }" id="price">${lesson.price }</span>
                </div>
            </div>
            <div class="orderInfo">
            	<div class="row">
    ...
    <div class="footer footer1">
    	...
    	<span class="price" id="pricess">${lesson.price}</span>
...
function validkd(){
	  ...
	  if(r.test(inp_kd)){
	    ...
	    if(inp_kz=="") inp_kz=0;
	    var to_pay='${lesson.price}'-((parseFloat(inp_kd)+parseFloat(inp_kz))/percent);
	    if(to_pay<0){alertN("使用积分不能超过课程金额");$("#kd_score").val("");return;}
	    $("#pricess").html(to_pay);
	  }else{
	      alertN("请输入正整数");
	      $("#kd_score").val("");
...
function validkz(){
	...
	function rows(){         
	   ...
	   $.post("${ctx}/wapapi/sms/lessonCode",datas,function(data){
	      if(data.code=="0"){ 
	    	  time();
	...
	$(".buyBtn").click(function(){
	  var kd_score=$("#kd_score").val();
	   ...
	   if(total_score>0&&(total_score!=(parseFloat(${lesson.price})*percent))){
	       alertN("积分金额必须订单总额");
	       return;
	   }
	   if($("#mobile").val()==""){
	   ...
	   $.post("${ctx}/shopapi/lessonOrder",{"kd_score":kd_score,"kz_score":kz_score,"mobile":$("#mobile").val(),"code":$("#code").val(),"conn_id":$("#id").val(),"type":0,"refer_number":$("#refer_number").val()},function(data){
