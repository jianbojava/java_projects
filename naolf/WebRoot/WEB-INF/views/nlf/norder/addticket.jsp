...
<body>
                        ...
                        <form action="" method="post" class="form-horizontal">
                         <c:if test="${finish==1}">
                            ...
                            <div class="form-group">
	                          ...
	                          <c:if test="${finish==0}">
		                         <div class="form-group">
	                                <label class="col-sm-2 control-label">卡券号</label>
	
	                                <div class="col-sm-9">
	                                    <select data-placeholder="选择卡券..." class="chosen-select" tabindex="2" name="ticket_sn">
				                          </select>
	                                </div>
	                            </div>
                            </c:if>
    ...
    <script>
        ...
        function selectTicket(){
	        if(${finish}==0){//添加订单才加载，历史订单不加载
		           var refer_number=$("[name='user'] option:selected").attr("tag");
		           var ticket_id=$("[name='ticket']").val();
		           $("[name='ticket_sn']").html("<option value=\"\">请选择卡券号</option>"); 
		           $(".chosen-select").trigger("chosen:updated");
		           if(refer_number!=""&&ticket_id!=""){
		               $.post("${ctx}/manage/userticket/query",{"number":refer_number,"conn_id":ticket_id,"used":0},function(data){
			                 for(var i=0;i<data.length;i++){
			        			$("[name='ticket_sn']").append("<option value=\""+data[i].sn+"\" hassubinfo=\"true\">"+data[i].sn+"</option>");
			        		}
			        		$(".chosen-select").trigger("chosen:updated");
		               })
		           }
        ...
        function add(){
           ...
           if($("[name='refer_number']").val()==""){
            ...
            var ticket_sn=$("[name='ticket_sn']").val();
			var params={"user_id":user_id,"conn_id":conn_id,"type":1,"ticket_sn":ticket_sn};
			var post_url="${ctx}/manage/norder/add";
			if(${finish}==1){//添加已结业历史订单
			   post_url="${ctx}/manage/norder/addfinish";
			   params={"user_id":user_id,"conn_id":conn_id,"type":1,"buy_type":$("[name='buy_type']:checked").val(),"create_date":$("[name='create_date']").val()};
			}
