     ...
     var r = /^\+?[1-9][0-9]*$/;//正整数验证
       function add(){
	       ...
	       if(!r.test(age)){
			...
			var params={"user_id":"${user_id}","name":$("[name='name']").val(),"school":$("[name='school']").val(),"age":$("[name='age']").val(),"grade":$("[name='grade']").val(),"gender":$("[name='gender']:checked").val()};
            $(".btn-primary").attr("disabled",true);//防止多次提交
            $.post("${ctx}/manage/student/add",params,function(data){
					if(data.code=="0"){
						opt_success("添加成功",0);
