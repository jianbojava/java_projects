    ...
    <script>
        ...
        function add(){
           ...
           if(minimum==""||maximum==""||isNaN(minimum)||isNaN(maximum)||parseInt(minimum)>=parseInt(maximum)){
			...
			var params={"name":$("[name='name']").val(),"remark":$("[name='remark']").val(),"minimum":parseInt(minimum),"maximum":parseInt(maximum),"image":$(".FildImg img").attr("src")};
			$.post("${ctx}/manage/level/add",params,function(data){
				if(data.code=="0"){
					opt_success("添加成功",0);
				}else{
					opt_error("添加失败");
