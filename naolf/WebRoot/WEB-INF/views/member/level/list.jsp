                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:level:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增等级','${ctx}/manage/level/add','','','')"><i class="fa fa-plus"></i> 新增等级</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="manage:level:del">
                                <div class="btn-group">
							...
							<form action="${ctx }/manage/level/list" method="post" id="searchForm" class="pull-right mail-search" target='_self'>
		                        <div class="input-group">
		                            <input type="text" class="form-control input-sm" name="keywords" value="${vo.keywords}" placeholder="搜索等级名称">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tbody>
                                <c:forEach items="${page.result }" var="level">
                                <tr>
                                	<th width="30"><input type="checkbox" class="i-checks" name="input[]" value="${level.id}"></th>
                                	<td>${level.name }</td>
                                	<td>${level.remark }</td>
                                	<td>${level.minimum}至${level.maximum}</td>
				                    <td><fmt:formatDate value="${level.create_date}" pattern="yyyy-MM-dd hh:mm:ss"/></td>
                                    <td><c:choose><c:when test="${level.enabled==0}"><span class="label label-primary">有效</span></c:when><c:otherwise><span class="label label-danger">无效</span></c:otherwise></c:choose></td>
                                    <td>
                                    	<shiro:hasPermission name="manage:level:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改等级','${ctx}/manage/level/update/${level.id}','','','')"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    	<shiro:hasPermission name="manage:level:del">
                                    	<div class="btn-group">
			                                <a data-toggle="dropdown" class="btn btn-xs btn-white dropdown-toggle"><i class="fa fa-cog"></i> 操作 <span class="caret"></span></a>
			                                <ul class="dropdown-menu">
			                                    <li><a onclick="del(0,'${level.id}')" <c:if test="${level.enabled==0}">class="font-bold"</c:if>>正常</a></li>
			                                    <li><a onclick="del(1,'${level.id}')" <c:if test="${level.enabled==1}">class="font-bold"</c:if>>无效</a></li>
			                                </ul>
			                            </div>
			                            </shiro:hasPermission>
                                    </td>
    ...
    <script>
        ...
        function del(flg,id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/level/del/"+flg+"/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else{
        ...
        function delM(flg){
        	if($("[name='input[]']").filter(':checked').length>0){
        		layer.confirm('确定要操作吗？', function(index){
					  ...
					  $.post("${ctx}/manage/level/del/"+flg+"/"+dels.substring(1),function(data){
						if(data.code=="0"){
							opt_success("操作成功");
						}else{
							opt_error("系统繁忙，请稍后再试");
