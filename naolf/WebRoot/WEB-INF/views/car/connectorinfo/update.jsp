...
<body>
                    ...
                    <div class="ibox-content">
                        <form action="" method="post" class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" disabled="disabled" name="connectorID" value="${c.connectorID }" placeholder="接口编号">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="connectorName" value="${c.connectorName }" placeholder="接口名称">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="voltageUpperLimits" value="${c.voltageUpperLimits }" placeholder="电压上线">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="voltageLowerLimits" value="${c.voltageLowerLimits }" placeholder="电压下线">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="current" value="${c.current }" placeholder="额定电流">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="power" value="${c.power }" placeholder="额定功率">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="parkNo" value="${c.parkNo }" placeholder="车位号">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control m-b" name="status">
                                        <option value="1" <c:if test="${c.status==1 }">selected</c:if>>空闲</option>
                                        <option value="2" <c:if test="${c.status==2 }">selected</c:if>>占用（未充电）</option>
                                        <option value="3" <c:if test="${c.status==3 }">selected</c:if>>占用（充电中）</option>
                                        <option value="4" <c:if test="${c.status==4 }">selected</c:if>>占用（预约锁定）</option>
                                        <option value="0" <c:if test="${c.status==0 }">selected</c:if>>离网</option>
                                        <option value="255" <c:if test="${c.status==255 }">selected</c:if>>故障</option>
                                    </select>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
    ...
    <script>
    	...
    	$("[name='stationID']").val("${c.stationID}");
    	getEquipments("${c.stationID}");
    	$("[name='stationID']").change(function(){
    		getEquipments($(this).val());
		})
		$(".chosen-select").trigger("chosen:updated");
        function add(){
			var params={"connectorID":"${c.connectorID}","connectorName":$("[name='connectorName']").val(),"connectorType":$("[name='connectorType']").val(),"voltageUpperLimits":$("[name='voltageUpperLimits']").val(),
        	...
        	$.post("${ctx}/manage/connectorinfo/update",params,function(data){
					if(data.code=="0"){
						opt_success("修改成功",0);
					}else{
						opt_error("修改失败");
		...
		function getEquipments(sid){
			...
			$.getJSON("${ctx}/manage/connectorinfo/getEquipments/"+sid,function(data){
				...
				$("[name='equipmentID']").val('${c.equipmentID}');
        		$(".chosen-select").trigger("chosen:updated");
			})
		}
    </script>
