                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:parking:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增车位','${ctx}/manage/parking/add','','','')"><i class="fa fa-plus"></i> 新增车位</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="manage:parking:del">
                                <div class="btn-group">
							...
							<form action="${ctx }/manage/parking/list" method="post" id="searchForm" class="pull-right mail-search">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tr>
                                    ...
                                    <td>${p.address }</td>
                                    <td>${p.dot_name }</td>
                                    <td><fmt:formatDate value="${p.sign_date }" pattern="yyyy-MM-dd"/>-<fmt:formatDate value="${p.end_date }" pattern="yyyy-MM-dd"/></td>
                                    <td>${p.dispatch_name }</td>
                                    <td><c:choose><c:when test="${p.enabled==0}"><span class="label label-primary">有效</span></c:when><c:otherwise><span class="label label-danger">无效</span></c:otherwise></c:choose></td>
                                    <td>
                                    	<shiro:hasPermission name="manage:parking:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改车位','${ctx}/manage/parking/update/${p.id }','','','')"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    	<shiro:hasPermission name="manage:parking:del">
                                    	<div class="btn-group">
			                                <a data-toggle="dropdown" class="btn btn-xs btn-white dropdown-toggle"><i class="fa fa-cog"></i> 操作 <span class="caret"></span></a>
			                                <ul class="dropdown-menu">
			                                    <li><a onclick="del(0,'${p.id}')" <c:if test="${p.enabled==0}">class="font-bold"</c:if>>正常</a></li>
			                                    <li><a onclick="del(1,'${p.id}')" <c:if test="${p.enabled==1}">class="font-bold"</c:if>>无效</a></li>
			                                </ul>
			                            </div>
			                            </shiro:hasPermission>
                                    </td>
    ...
    <script>
    	...
    	function del(flg,id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/parking/del/"+flg+"/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else{
        ...
        function delM(flg){
        	if($("[name='input[]']").filter(':checked').length>0){
        		layer.confirm('确定要操作吗？', function(index){
					  ...
					  $.post("${ctx}/manage/parking/del/"+flg+"/"+dels.substring(1),function(data){
						if(data.code=="0"){
							opt_success("操作成功");
						}else{
							opt_error("系统繁忙，请稍后再试");
