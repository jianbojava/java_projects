...
<body class="gray-bg">
                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		...
                        		<a class="btn btn-success btn-outline" onclick="layer_show('新增订单','${ctx}/manage/order/add','','',true)"><i class="fa fa-plus"></i> 新增订单</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="manage:order:export">
							...
							<form action="${ctx }/manage/order/list" method="post" id="searchForm"  class="pull-right" style="width: 845px">
		                        <div class="input-group">
		                            ...
		                            <select data-placeholder="选择调度员..." style="width:200px;" class="chosen-select" tabindex="2" name="dispatch_id">
		                            	<option value="">选择调度员...</option>
		                            	<c:forEach items="${disps }" var="d">
		                            ...
		                            <select data-placeholder="选择订单状态..." style="width:200px;" class="chosen-select" tabindex="2" name="status">
		                            	...
		                            	<option value="1">已备车</option>
		                            	<option value="2">已交车</option>
		                            	<option value="3">已还车</option>
		                            	<option value="4">已核价</option>
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tr>
                                    ...
                                    <td>
                                    	<c:choose>
                                    		<c:when test="${empty(o.dispatch_name) }">-</c:when>
                                    		<c:otherwise>${o.dispatch_name }</c:otherwise>
                                    	</c:choose>
                                    </td>
                                    <td>
                                    <c:choose>
                                    ...
                                    <td>
                                    	<shiro:hasPermission name="manage:order:view">
                                    	...
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('','${ctx}/manage/order/update/${o.id }','','',true)"><i class="fa fa-pencil"></i> 修改</a>
                                   		...
                                   		<shiro:hasPermission name="manage:order:del_flg">
			                            <a class="btn btn-xs btn-white" onclick="del_f('${o.id}')"><i class="fa fa-trash"></i> 删除</a>
                                    	</shiro:hasPermission>
                                    </td>
                                </tr>
                                </c:forEach>
    ...
    <script>
        ...
        function del_f(id){
        	layer.confirm('删除后不允许再做任何操作，确定要删除吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/order/del_flg/"+id,function(data){
				if(data.code=="0"){
					opt_success("删除成功");
				}else{
