                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:brand:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增品牌','${ctx}/manage/brand/add/${vo.type }','','','')"><i class="fa fa-plus"></i> 新增品牌</a>
                                </shiro:hasPermission>
                                <shiro:hasPermission name="manage:brand:del">
                                <div class="btn-group">
							...
							<form action="${ctx }/manage/brand/list" method="post" id="searchForm" class="pull-right mail-search" target='_self'>
		                        <div class="input-group">
		                            <input type="text" class="form-control input-sm" name="keywords" value="${vo.keywords}" placeholder="搜索品牌名称">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tr>
                                	<th><input type="checkbox" class="i-checks" name="input[]" value="${brand.id}"></th>
                                	<td>${brand.name }</td>
                                	<td>${brand.type_names }</td>
				                    <td><c:choose><c:when test="${brand.enabled==0}"><span class="label label-primary">正常</span></c:when><c:otherwise><span class="label label-danger">无效</span></c:otherwise></c:choose></td>
                                    <td>
                                    	<shiro:hasPermission name="manage:brand:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改品牌','${ctx}/manage/brand/update/${brand.id}/${vo.type }','','','')"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    	<shiro:hasPermission name="manage:brand:del">
                                    	<div class="btn-group">
			                                <a data-toggle="dropdown" class="btn btn-xs btn-white dropdown-toggle"><i class="fa fa-cog"></i> 操作 <span class="caret"></span></a>
			                                <ul class="dropdown-menu">
			                                    <li><a onclick="del(0,'${brand.id}')" <c:if test="${brand.enabled==0}">class="font-bold"</c:if>>正常</a></li>
			                                    <li><a onclick="del(1,'${brand.id}')" <c:if test="${brand.enabled==1}">class="font-bold"</c:if>>无效</a></li>
			                                </ul>
			                            </div>
			                            </shiro:hasPermission>
                                    </td>
    ...
    <script>
    	...
    	function del(flg,id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/brand/del/"+flg+"/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else{
	         ...
	         if($("[name='input[]']").filter(':checked').length>0){
	              layer.confirm('确定要操作吗？', function(index){
					  ...
					  $("[name='input[]']").filter(':checked').each(function(){
					   ...
					   $.post("${ctx}/manage/brand/del/"+flg+"/"+dels.substring(1),function(data){
						if(data.code=="0"){
							opt_success("操作成功");
						}else{
							opt_error("系统繁忙，请稍后再试");
