                    ...
                    <div class="tab-content">
                        <div id="tab-1" class="tab-pane active">
                        	<div class="wrapper wrapper-content">
			                        ...
			                        <c:choose>
                        				<c:when test="${order.status>=1 and order.pay_status==1 }">
                        				<div class="row" style="margin-bottom: 20px;">
				                        	<div class="col-sm-12">
				                        		<c:if test="${order.status==1}">
						                        <button type="button" class="btn btn-outline btn-primary" onclick="updateStatus(2)">对账</button>
						                        </c:if>
				                            </div>
				                        </div>
                        				</c:when>
                        			    <c:otherwise>
                        			    	<div class="alert alert-warning">订单未完成，无法对账</div>
                        			    </c:otherwise>
                        			</c:choose>
				                    ...
				                    <div class="well m-t"><textarea class="form-control" name="remark" style="resize: none" placeholder="订单备注">${order.remark }</textarea><a class="btn btn-white btn-bitbucket" onclick="updateOrder('remark',this)"><i class="fa fa-check"></i> 确定</a></div>
			            	</div>
                        </div>
    ...
    <script>
    	...
    	//修改状态
	  	function updateStatus(value){
			  ...
			  layer.close(index);
			  $.post("${ctx}/manage/charge/update?sn=${order.sn}&status="+value,function(data){
				if(data.code=="0"){
    	...
    	function updateOrder(field,o){
	  		...
	  		layer.confirm('确认要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/charge/update?sn=${order.sn}&"+field+"="+value,function(data){
				if(data.code=="0"){
					layer.msg("操作成功", {icon : 1});
					setTimeout("window.location.reload()", 1000)
				}else{
