                    ...
                    <div class="ibox float-e-margins">
                        ...
                        <div class="ibox-content">
                        	<div class="ibox-tools" style="float:left;">
                        		<shiro:hasPermission name="manage:operatorinfo:add">
                                <a class="btn btn-success btn-outline" onclick="layer_show('新增运营商','${ctx}/manage/operatorinfo/add','','','')"><i class="fa fa-plus"></i> 新增运营商</a>
                            	</shiro:hasPermission>
                            	<shiro:hasPermission name="manage:operatorinfo:del">
                            	<div class="btn-group">
							...
							<form action="${ctx }/manage/operatorinfo/list" method="post" id="searchForm" class="pull-right mail-search">
                            ...
                            <table class="footable table table-stripped toggle-arrow-tiny" data-sort="false" data-page-size="${vo.pageSize }">
                                ...
                                <tr>
                                	<td><input type="checkbox" class="i-checks" name="input[]" value="${o.operatorID }"></td>
                                	<td>${o.operatorID}</td>
                                	<td>${o.operatorName }</td>
                                	<td>
                                		<c:choose>
                                			<c:when test="${empty(o.operatorTel1) }">-</c:when>
                                			<c:otherwise>${o.operatorTel1 }</c:otherwise>
                                		</c:choose>
                                	</td>
                                	<td>
                                		<c:choose>
                                			<c:when test="${empty(o.operatorTel2) }">-</c:when>
                                			<c:otherwise>${o.operatorTel2 }</c:otherwise>
                                		</c:choose>
                                	</td>
                                    <td>
                                    	<c:choose>
                                			<c:when test="${empty(o.operatorRegAddress) }">-</c:when>
                                			<c:otherwise>${o.operatorRegAddress }</c:otherwise>
                                		</c:choose>
                                   	</td>
                                    <td>
                                    	<c:choose>
                                    		<c:when test="${o.enabled==0 }"><span class="label label-primary">正常</span></c:when>
                                    		<c:otherwise><span class="label label-danger">无效</span></c:otherwise>
                                    	</c:choose>
                                    </td>
                                    <td>${o.operatorNote }</td>
                                    <td>
                                    	<shiro:hasPermission name="manage:operatorinfo:update">
                                    	<a class="btn btn-xs btn-white" onclick="layer_show('修改运营商','${ctx}/manage/operatorinfo/update/${o.operatorID }','','','')"><i class="fa fa-pencil"></i> 修改</a>
                                    	</shiro:hasPermission>
                                    	<shiro:hasPermission name="manage:operatorinfo:del">
                                    	<div class="btn-group">
			                                <a data-toggle="dropdown" class="btn btn-xs btn-white dropdown-toggle"><i class="fa fa-cog"></i> 操作 <span class="caret"></span></a>
			                                <ul class="dropdown-menu">
			                                    <li><a onclick="del(0,'${o.operatorID}')" <c:if test="${o.enabled==0}">class="font-bold"</c:if>>正常</a></li>
			                                    <li><a onclick="del(1,'${o.operatorID}')" <c:if test="${o.enabled==1}">class="font-bold"</c:if>>无效</a></li>
			                                </ul>
			                            </div>
			                            </shiro:hasPermission>
                                    </td>
    ...
    <script>
    	...
    	function del(flg,id){
        	layer.confirm('确定要操作吗？', function(index){
			  layer.close(index);
			  $.post("${ctx}/manage/operatorinfo/del/"+flg+"/"+id,function(data){
				if(data.code=="0"){
					opt_success("操作成功");
				}else{
        ...
        function delM(flg){
        	if($("[name='input[]']").filter(':checked').length>0){
        		layer.confirm('确定要操作吗？', function(index){
					  ...
					  $.post("${ctx}/manage/operatorinfo/del/"+flg+"/"+dels.substring(1),function(data){
						if(data.code=="0"){
							opt_success("操作成功");
						}else{
							opt_error("系统繁忙，请稍后再试");
